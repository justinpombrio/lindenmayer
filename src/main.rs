...
use arith::{Bounds, Point, interpolate};
use canvas::Canvas;
use color_scale::{Color, color_scale_from_data, hilbert_color, hsl, orbit, rgb, sawtooth, scale};
...
fn main() {
    ...
    // Parse command line args (sets the above options)
    {
        ...
        args.refer(&mut background).add_option(
            &["--bg", "--background"],
            Parse,
            "Make the background color this color (default d2d2d2)",
        );
        args.refer(&mut foreground).add_option(
            &["--fg", "--foreground"],
            Parse,
            "If set, blend between the --background color at the center of the image, and this --foreground color at the edges",
        );
        ...
        args.refer(&mut fill_color).add_option(
            &["--fill"],
            Parse,
            "Fill inside the curve with this color, if set",
        );
        // Border
    ...
    match curve_style {
        ...
        CurveStyle::Straight => {
            ...
            for (i, end) in points.enumerate() {
                let color = color_scale(i as f64 / curve_len as f64);
                canvas.draw_circle(start, curve_width, color);
                canvas.draw_curve(|f| interpolate(f, start, end), curve_width, color);
                start = end;
            }
            canvas.draw_circle(start, curve_width, color_scale(1.0)); // start is now end
        }
        CurveStyle::Curvy => {
            let mut start = points.next().unwrap();
